...
# mutable to allow modification of the dynamics functions
mutable struct StateEstimationProblem
    model::ODESystem
    iosys::ODESystem
    inputs::Vector
    outputs::Vector
    disturbance_inputs::Vector
    state::Vector
    nx::Int
    nu::Int
    ny::Int
...
```
"""
function StateEstimationProblem(model, inputs, outputs; disturbance_inputs, discretization, Ts, df, dg, x0map=[], pmap=[])
    ...
    f, x_sym, ps, iosys = generate_control_function(model, inputs; simplify = true, split=false, force_SA=true, disturbance_inputs, disturbance_argument=false);
    f_aug, _ = generate_control_function(model, inputs; simplify = true, split=false, force_SA=true, disturbance_inputs, disturbance_argument=true);
    ...
    fd = discretization(f[1].f_oop, Ts, nx-na, na, nu)
    fd_aug = discretization(f_aug[1].f_oop, Ts, nx-na, na, nu)

    g = build_explicit_observed_function(iosys, outputs; inputs, disturbance_inputs, disturbance_argument=false)
    ...
    d0 = SimpleMvNormal(x0, 1e-8*I(nx)) # TODO: don't hardcode covariance

    StateEstimationProblem(model, iosys, inputs, outputs, disturbance_inputs, x_sym, nx, nu, ny, nw, na, fd, fd_aug, g.f_oop, ps, p, df, dg, d0)
end
...
function get_filter(prob::StateEstimationProblem, ::Type{ExtendedKalmanFilter}; constant_R1=true, kwargs...)
    R1mat = prob.df.Σ
    w0 = SVector(zeros(prob.nw)...) # typy instability here
    R1 = function (x,u,p,t)
        Bw = ForwardDiff.jacobian(w->prob.f_aug(x, u, p, t, w), w0)
        Bw * R1mat * Bw'
    end
    if constant_R1
    ...
    ExtendedKalmanFilter(prob.f, prob.g, R1, prob.dg.Σ, prob.d0; prob.nu, prob.ny, prob.nx, prob.p, kwargs...)
end

function get_filter(prob::StateEstimationProblem, ::Type{UnscentedKalmanFilter}; kwargs...)
    UnscentedKalmanFilter{false,false,true,false}(prob.f_aug, prob.g, prob.df.Σ, prob.dg.Σ, prob.d0; prob.nu, prob.ny, prob.nx, prob.p, kwargs...)
end
...
function propagate_distribution(f, kf::UnscentedKalmanFilter, x, args...; kwargs...)
    hasproperty(x, :μ) || error("Expected x to be a MvNormal or SimpleMvNormal")
    m,S = mean(x), cov(x)
    xs = LowLevelParticleFilters.sigmapoints(m, S, kf.weight_params)
